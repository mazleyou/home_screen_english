 ...
 */
object HSEInfoStateDefinition : GlanceStateDefinition<HSEInfo> {
    ...
    override suspend fun getDataStore(context: Context, fileKey: String): DataStore<HSEInfo> {
     ...
     */
    object WeatherInfoSerializer : Serializer<HSEInfo> {
        override val defaultValue = HSEInfo.Unavailable("no place found")

        override suspend fun readFrom(input: InputStream): HSEInfo = try {
        ...
        override suspend fun writeTo(t: HSEInfo, output: OutputStream) {
            output.use {
                it.write(
                    Json.encodeToString(HSEInfo.serializer(), t).encodeToByteArray()
                )
            }
        }
    }
